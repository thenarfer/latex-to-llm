...
def collect_dependencies(entry_files, ignore_folders, ignore_files):
    ...
    def recurse(path):
        ...
        if matches_any(rel, ignore_files):
            return
        if any(rel.startswith(p.rstrip("*/") + os.sep) for p in ignore_folders):
            return
        ...
        for ref in INCLUDE_REGEX.findall(text):
            child = resolve_tex_path(os.path.dirname(path), ref)
            child_rel = os.path.relpath(child)
        ...
        for ref in BIB_REGEX.findall(text):
            ...
            bibs.add(os.path.normpath(os.path.join(os.path.dirname(path), bib_file)))
